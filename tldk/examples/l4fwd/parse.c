	...
	char *fecfg_fname, char *becfg_fname)
{
	...
	printf("cfg->prt[i].id = %u;\n", cfg->prt[i].id);
	printf("cfg->prt[i].nb_lcore = %u;\n", cfg->prt[i].nb_lcore);
	printf("cfg->prt[i].mtu = %u;\n", cfg->prt[i].mtu);
	printf("cfg->prt[i].rx_offload = %x;\n", cfg->prt[i].rx_offload);
	printf("cfg->prt[i].tx_offload = %x;\n", cfg->prt[i].tx_offload);
	printf("cfg->prt[i].ipv4 = %u;\n", cfg->prt[i].ipv4);
	printf("cfg->prt[i].hash_key_size = %u;\n", cfg->prt[i].hash_key_size);
	printf("cfg->prt[i].id = %u;\n", cfg->prt[i].id);
	}
