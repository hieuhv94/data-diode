...
static int
sync_ack(struct tle_tcp_stream *s, uint32_t r_pidx,  uint32_t o_pidx, const union pkt_info *pi,
	const union seg_info *si, uint32_t ts, struct rte_mbuf *m)
{
	...
	if (type == TLE_V4)
	{
		da = &pi->addr4.src;
		sa = &pi->addr4.dst;
		lc = s->s.ctx->prm.lookup4_data;
		// Add dest ip

		printf("Add %x to port %u\n", pi->addr4.src, r_pidx);
		rc = s->s.ctx->prm.add_dest4(s->s.ctx->prm.lookup4_data, da, pi->src_mac, r_pidx, o_pidx, 1);
	}
	else
	{
		da = &pi->addr6->src;
		sa = &pi->addr6->dst;
		lc = s->s.ctx->prm.lookup6_data;
	}
	printf("SYNC ACK %x %x\n",pi->addr4.src, pi->addr4.dst);
	
	rc = stream_get_dest(&s->s, da, sa, &dst);
	if (rc < 0)
		return rc;
...
stream_fill_dest(struct tle_tcp_stream *s)
{
	...
	rc = stream_get_dest(&s->s, da, sa, &s->tx.dst);
	return (rc < 0) ? rc : 0;
}
...
static inline uint32_t
rx_syn(struct tle_dev *dev, uint32_t r_pidx,  uint32_t o_pidx, uint32_t type, uint32_t ts,
	const union pkt_info pi[], const union seg_info si[],
	struct rte_mbuf *mb[], struct rte_mbuf *rp[], int32_t rc[],
	uint32_t num)
{
	...
	for (i = 0; i != num; i++) {
		...
		if (rx_check_stream(s, &pi[i]) != 0)
			...
			ret = sync_ack(s, r_pidx, o_pidx, &pi[i], &si[i], ts, mb[i]);
...
uint16_t
tle_tcp_rx_bulk(struct tle_dev *dev, uint32_t r_pidx, uint32_t o_pidx, struct rte_mbuf *pkt[],
	struct rte_mbuf *rp[], int32_t rc[], uint16_t num)
{
	...
	for (i = 0; i != num; i += j) {
		...
		/* process input SYN packets */
			...
			n = rx_syn(dev, r_pidx, o_pidx, t, ts, pi + i, si + i, pkt + i,
				rp + k, rc + k, j);
			k += j - n;
		} else {
