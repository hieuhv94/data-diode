...
static int ConfigSetTx(DPDKIfaceConfig *iconf, int entry_bool);
...
#define DPDK_CONFIG_DEFAULT_MAC_ADDRESS                 "ff:ff:ff:ff:ff:ff"
...
static int ConfigSetMacAddress(DPDKIfaceConfig *iconf, const char *entry_str)
{
    ...
    if (entry_str == NULL || entry_str[0] == '\0')
    {
        SCLogError("Mac address in DPDK config is NULL or empty");
        SCReturnInt(-EINVAL);
    }
    unsigned char ptr[6];
    int last = -1;
    int rc = sscanf(entry_str, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx%n",
                    ptr + 0, ptr + 1, ptr + 2, ptr + 3, ptr + 4, ptr + 5,
                    &last);
    if(rc != 6)
    {
        SCLogError("Invalid mac address format %s", entry_str);
        SCReturnInt(-EINVAL);
    }
    iconf->mac_addr =   (uint64_t)(ptr[5]) << 40 |
                        (uint64_t)(ptr[4]) << 32 | ( 
...
static int ConfigSetHostMacAddress(DPDKIfaceConfig *iconf, const char *entry_str)
{
    ...
    if (entry_str == NULL || entry_str[0] == '\0')
    {
        SCLogError("Host mac address in DPDK config is NULL or empty");
        SCReturnInt(-EINVAL);
    }
    ...
    if(rc != 6)
    {
        SCLogError("Invalid host mac address format %s", entry_str);
        SCReturnInt(-EINVAL);
    }
    iconf->host_mac_addr =  (uint64_t)(ptr[5]) << 40 |
                            (uint64_t)(ptr[4]) << 32 | ( 
                            // 32-bit instructions take fewer bytes on x86, so use them as much as possible.
                            (uint32_t)(ptr[3]) << 24 | 
                            (uint32_t)(ptr[2]) << 16 |
...
static int ConfigLoad(DPDKIfaceConfig *iconf, const char *iface)
{
    ...
    if (retval < 0)
        SCReturnInt(retval);
    retval = ConfGetChildValueBoolWithDefault(
                     if_root, if_default, dpdk_yaml.tx, &entry_bool) != 1
                     ? ConfigSetTx(iconf, 1)
                     : ConfigSetTx(iconf, entry_bool);
    if (retval < 0)
        SCReturnInt(retval);
    retval = ConfGetChildValueWithDefault(if_root, if_default, dpdk_yaml.mac_addr, &entry_str) != 1
                     ? ConfigSetMacAddress(iconf, DPDK_CONFIG_DEFAULT_MAC_ADDRESS)
                     : ConfigSetMacAddress(iconf, entry_str);
    if (retval < 0)
        SCReturnInt(retval);
    retval = ConfGetChildValueWithDefault(if_root, if_default, dpdk_yaml.host_mac_addr, &entry_str) != 1
                     ? ConfigSetHostMacAddress(iconf, DPDK_CONFIG_DEFAULT_MAC_ADDRESS)
