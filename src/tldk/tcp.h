...
netfe_lcore_init_tcp(const struct netfe_lcore_prm *prm)
{
	...
	lcore = 2;//rte_lcore_id();
	snum = prm->max_streams;
	printf("%s(lcore=%u, nb_streams=%u, max_streams=%u)\n",
		__func__, lcore, prm->nb_streams, snum);

	memset(&eprm, 0, sizeof(eprm));
	eprm.socket_id = 0;//rte_lcore_to_socket_id(lcore);
	...
	printf("%s(%u) synevq=%p, erevq=%p, rxevq=%p, txevq=%p\n",
