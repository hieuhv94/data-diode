 ...
 */
static int
port_init(struct netbe_port *uprt, uint32_t proto, uint8_t is_forcing)
{
	int32_t rc;
...
netbe_port_init(struct netbe_cfg *cfg)
{
	...
	for (i = 0; i != cfg->prt_num; i++) {
		...
		for (j = 0; j < prt->nb_lcore; j++) {
			...
			sid = 1;//rte_lcore_to_socket_id(prt->lcore_id[j]) + 1;
...
static int
netbe_port_add(struct netbe_lcore *lc, struct netbe_port *prt, uint32_t proto, uint32_t promisc, uint32_t mpool_buf_num, uint32_t* prtid)
{
	int32_t rc;
	uint32_t i, sid, j;

	rc = port_init(prt, proto, 1);
		if (rc != 0) {
			RTE_LOG(ERR, USER1,
				"%s: port=%u init failed with error code: %d\n",
				__func__, prt->id, rc);
		...
		for (j = 0; j < prt->nb_lcore; j++) {
			...
			if (mpool[sid] == NULL) {
				rc = pool_init(sid, mpool_buf_num);
				if (rc != 0)
					return rc;
			}

			if (frag_mpool[sid] == NULL) {
				rc = frag_pool_init(sid, mpool_buf_num);
				if (rc != 0)
					return rc;
			}
