...
pkt_buf_fill(uint32_t lcore, struct pkt_buf *pb, uint32_t dlen)
{
	...
	sid = 1;//rte_lcore_to_socket_id(lcore) + 1;
	...
	uint8_t idx)
{
	...
	lc->dest_map[idx] = addr;
	// rc = rte_lpm_add(lc->lpm4, addr, depth, idx);
	RTE_LOG(INFO, USER1, "%s(idx=%u addr=%x lcore=%u,port=%u,dev=%p,"
		"ipv4=%s/%u,mtu=%u,"
		"mac=%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx) "
		"returns %d;\n",
		__func__, idx, addr,lc->id, dst->port, lc->dst4[idx].dev,
...
static void
netbe_l2fwd_mac_updating(struct rte_mbuf *m, uint64_t src_addr, uint64_t dst_addr)
{
	struct rte_ether_hdr *eth;
	printf("BE mac update\n");
	eth = rte_pktmbuf_mtod(m, struct rte_ether_hdr *);
...
netbe_tx(struct netbe_lcore *lc, uint32_t pidx)
{
	...
	if (k >= RTE_DIM(lc->prtq[pidx].tx_buf.pkt) / 2) {
		j = tle_tcp_tx_bulk(lc->prtq[pidx].dev, mb + n, k);
		n += j;
...
netfe_rx_process(uint32_t lcore, struct netfe_stream *fes)
{
	...
	n = tle_tcp_stream_recv(fes->s, fes->pbuf.pkt + n, k);
	if (n == 0)
		return 0;
